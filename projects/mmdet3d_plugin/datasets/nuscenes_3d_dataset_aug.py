...
class NuScenes3DDatasetAug(Dataset):
    DefaultAttribute = {
        "car": "vehicle.parked",
        ...
        if self.modality["use_camera"]:
            ...
            for cam_type, cam_info in info["cams"].items():
                ...
                for filename in image_paths:
                    if self.test_mode:
                        # filename = filename.replace('samples', 'images_orig')
                        continue
                    else:
                        new_filename = filename.replace('./data/nuscenes', './data/nuscenes_aug')
                        if os.path.isfile(new_filename) and (random.random() > 0.3):

...
def get_forward_config(config, mode='train', weight_path='./', novel_view_mode='SF'):
    """
...
class NuScenes3DDataset(Dataset):
    ...
    def __init__(
        ...
        self.novel_view_config = novel_view_config
        
        
        # import pdb; pdb.set_trace()
        ### driving forward config
        if not self.test_mode:
            
            _augmentation = {
            # 'image_shape': (int(data_aug_conf['final_dim'][0]), int(data_aug_conf['final_dim'][1])), 
            # 'image_shape': (int(data_aug_conf['H']), int(data_aug_conf['W'])), 
            'image_shape': (), 
            'jittering': (),
            'crop_train_borders': (),
            'crop_eval_borders': ()
            }
            cfg = get_forward_config('./driving_forward.yaml', mode='train')
            self.driving_forward = construct_dataset(cfg, 'train', **_augmentation)
            ...
            cfg = get_forward_config('./driving_forward.yaml', mode='eval')
            self.driving_forward = construct_dataset(cfg, 'val', **_augmentation)
    ...
    def _set_sequence_group_flag(self):
        ...
        prev_scene_token = ''
        for idx in range(len(self.data_infos)):
            if idx != 0 and (prev_scene_token != self.data_infos[idx]["scene_token"]):
                # Not first frame and # of sweeps is 0 -> new sequence
                curr_sequence += 1
            prev_scene_token = self.data_infos[idx]["scene_token"]
            res.append(curr_sequence)
    ...
    def __getitem__(self, idx):
        ...
        # dict_keys(['token', 'map_location', 'pts_filename', 'sweeps', 'timestamp', 
        # 'lidar2ego_translation', 'lidar2ego_rotation', 'ego2global_translation', 'ego2global_rotation', \
        # 'ego_status', 'map_infos', 'lidar2global', 'map_geoms', 'img_filename', 'lidar2img', 
        # 'lidar2cam', 'cam_intrinsic', 'gt_bboxes_3d', 'gt_labels_3d', 'gt_names', 'instance_inds',
        # 'gt_agent_fut_trajs', 'gt_agent_fut_masks', 'gt_ego_fut_trajs', 'gt_ego_fut_masks', 
        # 'gt_ego_fut_cmd', 'fut_boxes', 'aug_config'])
        # import copy; data_orig = copy.deepcopy(data)
        # intrinsic = data['cam_intrinsic'][0]
        # viewpad = np.eye(4)
        # viewpad[: intrinsic.shape[0], : intrinsic.shape[1]] = intrinsic
        # lidar2img_rt = data['lidar2img'][0]
        # lidar2cam = (np.linalg.inv(viewpad) @ lidar2img_rt)
        
        # lidar2ego = np.eye(4)
        # lidar2ego[:3, :3] = pyquaternion.Quaternion(
        #     data['lidar2ego_rotation']
        # ).rotation_matrix
        # lidar2ego[:3, 3] = np.array(data['lidar2ego_translation'])
        # extrinsic = data['forward_ref']['extrinsics'][0]
        # cam2ego = lidar2ego @ inverse_T(lidar2cam)
        # temp = copy.deepcopy(data['forward_ref'])
        
        # add files
        # if not self.test_mode:
        data = self.driving_forward.preprocess(data)
        
        # import pdb; pdb.set_trace()
        data = self.pipeline(data)
        
        # if 'img' in data and (data['img'].data.shape[0] > 6):
        #     img = data['img'].data 
        #     if img.max() > 250:
        #         img = img / 255.0
        #     data['context_img'] = img[6:]
        #     data['img']  = DC(img[:6], stack=True)
        #     data['image_wh'] = data['image_wh'][:6]
        
        
        if 'img' in data and (data['img'].data.shape[0] > 6):
            img = copy.deepcopy(data['img'].data)
            data['img_orig'] = (data['img_norm_cfg']['mean'][None, :, None, None] + img.numpy() * data['img_norm_cfg']['std'][None, :, None, None]).clip(0.0, 255.0) / 255.0
            
            data['context_img'] = data['img'].data[6:]
            data['img']  = DC(data['img'].data[:6], stack=True)
            data['image_wh'] = data['image_wh'][:6]
        
        # if not self.test_mode:
        
        # context_img = out['img'].data[6:]
        # out['img']  = DC(out['img'].data[:6])
        data = self.driving_forward.postprocess(data)
        
        
        
        # dict_keys(['img_metas', 'img', 'timestamp', 'projection_mat', 'image_wh', 'gt_depth', 
        # 'focal', 'gt_bboxes_3d', 'gt_labels_3d', 'gt_map_labels', 'gt_map_pts', 
        # 'gt_agent_fut_trajs', 'gt_agent_fut_masks', 'gt_ego_fut_trajs', 'gt_ego_fut_masks', 
        # 'gt_ego_fut_cmd', 'ego_status'])
        ...
        if not self.test_mode:
            aug_data = data.copy()
            aug_data['forward'] = data['forward'].copy()
            
            aug_data.pop('img')
            aug_data['projection_mat'] = aug_data.pop('projection_mat_aug')
            gt_depth_aug = aug_data.pop('gt_depth_aug', None)
            if gt_depth_aug is not None: aug_data['gt_depth'] = gt_depth_aug
            
            # import pdb; pdb.set_trace()
            for k in list(data['forward'].keys()):
                if 'color' in k or 'color_aug' in k:
                    aug_data['forward'].pop(k)
                elif k == 'depth_aug':
                    depth_aug = aug_data['forward'].pop('depth_aug', None)
                    data['forward'].pop('depth_aug')
                    if depth_aug is not None: aug_data['forward']['depth'] = depth_aug
                elif 'cam_T_cam_aug' in k:
                    aug_data['forward']['cam_T_cam', k[1], k[2]] = aug_data['forward'].pop(k)
                    data['forward'].pop(k)
    ...
    def cam_pos_augmentor(self):
        
        # augment ['height', 'pitch', 'depth']
        # Towards Viewpoint Robustness in Bird’s Eye View Segmentation
        # import pdb; pdb.set_trace()
        # Large Car vs Small Car
        if self.novel_view_config is None:
            pitch_range = [-10, 5]
            height_range =  [-0.7, 1.1]
            depth_range = [-0.2, 1.0]
        else:
            pitch_range = self.novel_view_config.get('pitch', [-10, 5])
            height_range =  self.novel_view_config.get('height', [-0.7, 1.1])
            depth_range = self.novel_view_config.get('depth', [-0.2, 1.0])

        # height: meter
        height = 0.0
        while abs(height) < 0.1: 
            height = random.uniform(height_range[0], height_range[1])
        
        # pitch: degree
        if height > 0.0:
            pitch = random.uniform(pitch_range[0], -1)
        else: 
            pitch = random.uniform(1, pitch_range[1])
        
        # Depth augmentation
        
        front_back_depth = random.uniform(depth_range[0], depth_range[1])
        
        # pitch=0.0
        # height=0.0
        # front_back_depth=2.0
        
        angle_radians = np.radians(pitch)
        rotation_x = R.from_euler('x', angle_radians, degrees=False).as_matrix()
        
        translation = np.array([0.0, front_back_depth, height])     
        
        return rotation_x, translation
        ...
        if self.modality["use_camera"]:
            ...
            for cam_type, cam_info in info["cams"].items():
                ...
                if self.test_mode:
                    ...
                    if self.novel_view:
                        new_filename = filename.replace('samples', self.novel_view_config.get('img_dir', 'samples'))
                        # new_filename = filename
                        # import pdb; pdb.set_trace()
                        cam_delta = self.novel_view_config.get('cam_delta', [0.0, 0.0, 0.0])
                        height = cam_delta[0]
                        tilt = cam_delta[1]
                        ref_depth = cam_delta[2]
                    
                        if cam_type == 'CAM_FRONT':
                            depth = ref_depth
                        elif cam_type == 'CAM_FRONT_LEFT' or cam_type == 'CAM_FRONT_RIGHT':
                            depth = ref_depth / 2.0
                        elif cam_type == 'CAM_BACK_LEFT' or cam_type ==  'CAM_BACK_RIGHT':
                            depth = -ref_depth / 2.0
                            # import pdb; pdb.set_trace()
                        ...
                        if os.path.isfile(new_filename):
                            ...
                            print('Warning: ', new_filename, " does not exist!")
                    else:
                        height = 0.0
                        tilt = 0.0
                        depth = 0.0
                ...
                if self.test_mode:
                    angle_radians = np.radians(tilt)
                    rotation_x = R.from_euler('x', angle_radians, degrees=False).as_matrix()
                    
                    lidar2cam_r = np.linalg.inv(cam_info["sensor2lidar_rotation"] @ rotation_x)
                    lidar2cam_t = (
                        (cam_info["sensor2lidar_translation"] + np.array([0, depth, height])) @ lidar2cam_r.T
                    )
                else:
                ...
                ###########
                aug_translation_cam = copy.deepcopy(aug_translation)
                if cam_type == 'CAM_FRONT':
                    pass
                elif cam_type == 'CAM_FRONT_LEFT' or cam_type == 'CAM_FRONT_RIGHT':
                    aug_translation_cam[1] = aug_translation_cam[1] / 2.0
                elif cam_type == 'CAM_BACK_LEFT' or cam_type ==  'CAM_BACK_RIGHT':
                    aug_translation_cam[1] = - aug_translation_cam[1] / 2.0
                elif cam_type == 'CAM_BACK':
                    aug_translation_cam[1] = - aug_translation_cam[1]
                else:
                    raise NotImplementedError
                
                lidar2cam_r_aug = np.linalg.inv(cam_info["sensor2lidar_rotation"] @ aug_rotation)
                lidar2cam_t_aug = (
                    (cam_info["sensor2lidar_translation"] + aug_translation_cam) @ lidar2cam_r_aug.T
                )
                lidar2cam_rt_aug = np.eye(4)
                lidar2cam_rt_aug[:3, :3] = lidar2cam_r_aug.T
                lidar2cam_rt_aug[3, :3] = -lidar2cam_t_aug
                
                lidar2img_rt_aug = viewpad @ lidar2cam_rt_aug.T
                lidar2img_rts_aug.append(lidar2img_rt_aug)
                lidar2cam_rts_aug.append(lidar2cam_rt_aug)
                #####################
            ...
            input_dict.update(
                dict(
                    ...
                    lidar2img_aug=lidar2img_rts_aug,
                    lidar2cam_aug=lidar2cam_rts_aug,
    ...
    def _evaluate_single(
        ...
        if not tracking:
            from nuscenes.eval.detection.evaluate_custom import NuScenesEvalCustom as NuScenesEval
            # import pdb; pdb.set_trace()
            sample_tokens = [item['token'] for item in self.data_infos]
    ...
    def _evaluate_single_motion(self,
        ...
        """
        from nuscenes import NuScenes
        from .evaluation.motion.motion_eval_uniad_custom import NuScenesEval as NuScenesEvalMotion
    ...
    def evaluate(
        ...
        res_path = f"results_.pkl" if "trainval" in self.version else "results_mini.pkl"
        ...
        #         mask = result['img_bbox']['scores_3d'] >= det_thresh
        #         for key in ['boxes_3d', 'labels_3d', 'cls_scores', 'scores_3d']:
        #             result['img_bbox'][key] = result['img_bbox'][key][mask]
        ...
        if eval_mode['with_det']:
            ...
            for metric in ["detection", "tracking"]:
                ...
                # total_indices = set(range(len(self.data_infos)))  # 0부터 6019까지의 인덱스를 set으로 생성

                # # 제외할 인덱스 리스트 (예시)
                # # selected_indices = [1360, 1361, 1832, 1833, 1838, 1839, 1903, 1942, 1943, 2631, 2972, 2973, 3097, 3133, 3134, 3135, 3140, 3141, 3142, 3143, 3144, 3146, 3826, 4168, 4306, 4312, 5300, 5320, 5517, 5518, 5720] 
                # # selected_indices = [119, 401, 463, 464, 666, 667, 782, 1360, 1361, 1722, 1838, 1840, 1942, 2821, 2822, 2823, 2899, 2900, 3085, 3086, 3087, 3134, 3135, 3143, 3144, 3145, 3146, 4035, 4164, 4167, 4168, 4311, 4312, 4648, 4701, 4702, 4988, 5123, 5124, 5231, 5232, 5233, 5234, 5235, 5275, 5276, 5300, 5320, 5513, 5514, 5516, 5517, 5518]
                # selected_indices = [461, 463, 739, 1360, 1361, 1726, 1831, 1838, 1839, 1942, 1983, 2822, 3133, 3134, 3135, 3141, 3142, 3143, 3144, 3145, 3826, 4028, 4164, 4167, 4168, 4312, 5115, 5123, 5232, 5233, 5234, 5300, 5518, 5718, 5719, 5720, 5721]
                # # selected_indices = [727, 1360, 1361, 1668, 1831, 1832, 1833, 1834, 1838, 1942, 1954, 2821, 2822, 2915, 3091, 3092, 3096, 3122, 3135, 3141, 3143, 3144, 3145, 3146, 3825, 3827, 4035, 4167, 4168, 4311, 4312, 4415, 4420, 4422, 4530, 4984, 4987, 4988, 5014, 5232, 5233, 5234, 5240, 5245, 5246, 5300, 5301, 5320, 5555, 5556, 5557, 5558, 5559, 5560, 5561, 5562, 5563, 5564, 5566]
                # # selected_indices = [119, 120, 464, 667, 727, 1359, 1360, 1361, 1753, 1831, 1838, 1839, 1942, 2438, 2799, 2903, 3133, 3134, 3135, 3139, 3140, 3141, 3142, 3143, 3144, 3145, 3146, 3825, 4164, 4168, 4306, 4311, 4530, 4987,4988, 5123, 5124, 5231, 5232, 5233, 5234, 5235, 5236, 5300, 5518, 5760]
                # # selected_indices = [666, 667, 1359, 1360, 1361, 1833, 1839, 1942, 2438, 2631, 2799, 2822, 3083, 3133, 3134, 3141, 3142, 3143, 3144, 3145, 3146, 3824, 3825, 3826, 4164, 4168, 4312, 5123, 5232, 5233, 5234, 5235, 5300, 5517]
                # # selected_indices = [460, 464, 737, 738, 739, 1278, 1291, 1359, 1360, 1671, 1838, 1839, 1942, 2001, 2109, 2437, 2439, 3090, 3095, 3096, 3097, 3098, 3099, 3100, 3101, 3125, 4109, 4116, 4128, 4248, 4249, 4829, 5278, 5285, 5432, 5433, 5434, 5435, 5517, 5518, 5519, 5542, 5562, 5760, 5761, 5819] 
                # # selected_indices = [119, 122, 460, 555, 666, 738, 804, 805, 807, 808, 809, 810, 1112, 1231, 1232, 1233, 1291, 1832, 1833, 1837, 1838, 1903, 1942, 2331, 2332, 2691, 2695, 2820, 2821, 2822, 2823, 2824, 2825, 2826, 2903, 2932, 2933, 2946, 3094, 3095, 3096, 3097, 3098, 3099, 3100, 3101, 3102, 3103, 3104, 3105, 3106, 3107, 3108, 3109, 3121, 3123, 3124, 3125, 3126, 3127, 3128, 3144, 3184, 4035, 4168, 4310, 4311, 4648, 4700, 4701, 4702, 4703, 4708, 4985, 4986, 4987, 4988, 4989, 5203, 5230, 5231, 5232, 5233, 5234, 5275, 5276, 5277, 5301, 5320, 5466, 5467, 5468, 5469, 5489, 5514, 5517, 5518, 5520, 5521, 5527, 5536, 5537]
                # n=5
                # preceding_indices = sorted(set(i for idx in selected_indices for i in range(max(0, idx - n + 1), idx + 1)))

                # # selected_indices를 제외한 나머지 인덱스를 찾기
                # remaining_indices = list(total_indices - set(preceding_indices))
                
                # # import pdb; pdb.set_trace()
                # results = [ results[i] for i in remaining_indices]
                # self.data_infos = [self.data_infos[i] for i in remaining_indices]

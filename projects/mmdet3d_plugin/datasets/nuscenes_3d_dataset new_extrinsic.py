...
class NuScenes3DDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # data.pop('token', None)
        # import pdb; pdb.set_trace()
        if 'img' not in data: return data
    ...
    def cam_pos_augmentor(self):
        ...
        # pitch: degree
        if height > 0.0:
            pitch = random.uniform(-10, -1)
        else: 
            pitch = random.uniform(1, 5)
        
        # Depth augmentation
        
        front_back_depth = random.uniform(-0.2, 0.5)
    ...
    def get_data_info(self, index):
        ...
        if self.modality["use_camera"]:
            ...
            for cam_type, cam_info in info["cams"].items():
                ...
                camaug2ego = cam2ego.copy()
                camaug2ego[:3, :3] = camaug2ego[:3, :3] @ aug_rotation
                camaug2ego[:3, 3] = camaug2ego[:3, 3] + aug_translation_cam
                camaug2lidar = self.inverse_T(lidar2ego) @ camaug2ego
                cam2egos.append(cam2ego)
                camaug2egos.append(camaug2ego)
    ...
    def evaluate(
        ...
        if eval_mode['with_gaussian']:
            from .evaluation.gaussian.gaussian_eval import gaussian_eval
            gaussian_results_dict = gaussian_eval(results, self.eval_config, logger=logger)
            results_dict.update(gaussian_results_dict)

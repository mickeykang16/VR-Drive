...
class MultiScaleDepthMapGenerator(object):
    ...
    def __call__(self, input_dict):
        ...
        if 'lidar2img_aug' in input_dict:
            gt_depth = []
            for i, lidar2img in enumerate(input_dict["lidar2img_aug"]):
                H, W = input_dict["img_shape"][i][:2]
            ...
            input_dict["gt_depth_aug"] = [np.stack(x) for x in gt_depth]
...
class NuScenesSparse4DAdaptor(object):
    ...
    def __call__(self, input_dict):
        ...
        if "lidar2img_aug" in input_dict:
            input_dict["projection_mat_aug"] = np.float32(
                np.stack(input_dict["lidar2img_aug"])
            )

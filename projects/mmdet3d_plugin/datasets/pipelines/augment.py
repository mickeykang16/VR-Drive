...
class ResizeCropFlipImage(object):
    def __call__(self, results):
        ...
        if aug_config is None:
            return results
        imgs = results["img"]
        # results['img_orig'] = copy.deepcopy(results["img"])
        N = len(imgs)
        new_imgs = []
        NN = len(results["lidar2img"])
        for i in range(N):
            ...
            if i < NN:
                ...
                if "lidar2img_aug" in results:
                    # import pdb; pdb.set_trace()
                    results["lidar2img_aug"][i] = mat @ results["lidar2img_aug"][i]
        ...
        #         np.uint8(imgs[i]), aug_config,
        #     )
        #     context_img.append(np.array(img).astype(np.float32))
        ...
        if mask:
            new_mask = []
            for m in mask:
                img, _ = self._img_transform(
                    m, aug_config,
                )
                new_mask.append(img)
            results['mask'] = new_mask

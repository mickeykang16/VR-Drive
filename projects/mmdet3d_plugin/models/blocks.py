...
from .gaussian import DenseGaussianNet
...
class DenseDepthNet(BaseModule):
    ...
    def forward(self, feature_maps, inputs, focal=None, max_depth=80.0, min_depth=1.5, focal_length_scale=300, gt_depths=None):
        if focal is None:
        ...
        if feature_maps[0].dim() == 5:  # multi-view
            num_cams = feature_maps[0].shape[1]
            
        outputs = {}
        
        # dictionary initialize
        for cam in range(num_cams): # self.num_cames = 6
        ...
        for i, feat in enumerate(feature_maps[: self.num_depth_layers]):
            disp = self.sigmoid(self.depth_layers[i](upsample(feat.flatten(end_dim=1))))
            ref_K = inputs[('K', i)]
            disp = rearrange(disp, '(b nc) 1 h w -> b nc 1 h w', b=bs, nc=num_cams)
            depth = self.to_depth(disp, ref_K, max_depth, min_depth, focal_length_scale)
            # depth = depth.transpose(0, -1) * focal / self.equal_focal
            # depth = depth.transpose(0, -1)
            depths.append(rearrange(depth, 'b nc 1 h w -> nc b h w', b=bs, nc=num_cams).float())
        ...
        for cam in range(num_cams):
            for i in range(len(depths)):
                # import pdb; pdb.set_trace()
                outputs[('cam', cam)][('depth', 0, i)] = depths[i][cam, ...].unsqueeze(1)
    
        return depths, outputs

    def to_depth(self, disp_in, K_in, max_depth, min_depth, focal_length_scale):        
        """
        ...
        depth = 1/disp
        return depth * K_in[:, :, 0:1, 0:1].unsqueeze(2)/focal_length_scale
...
#             depth = depth.transpose(0, -1) * focal / self.equal_focal
#             depth = depth.transpose(0, -1)
#             depths.append(rearrange(depth, '(b nc) 1 h w -> nc b h w', b=bs, nc=num_cams))
...
#             for i in range(len(depths)):
#                 # import pdb; pdb.set_trace()
#                 outputs[('cam', cam)][('depth', 0, i)] = upsample(depths[i][cam, ...].unsqueeze(1))

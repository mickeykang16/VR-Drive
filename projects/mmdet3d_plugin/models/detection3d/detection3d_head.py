...
class Sparse4DHead(BaseModule):
    def __init__(
        ...
        **kwargs,
        ...
        # self.instance_bank1 = build(instance_bank, PLUGIN_LAYERS)
        # self.instance_bank2 = build(instance_bank, PLUGIN_LAYERS)
        
        # self.instance_bank2.anchor = self.instance_bank1.anchor
        # self.instance_bank2.instance_feature = self.instance_bank1.instance_feature
        
        
        # self.instance_bank3 = build(instance_bank, PLUGIN_LAYERS)
        ...
        self.op_config_map = {
            ...
            "deformable2": [deformable_model, ATTENTION],
    ...
    def forward(
        ...
        # if original_bank:
        #     self.instance_bank = self.instance_bank1
        # else:
        #     self.instance_bank = self.instance_bank2
        # self.instance_bank = self.instance_bank1
        ...
        if not "deformable2" in self.operation_order:
            instance_loss = None
        ...
        for i, op in enumerate(self.operation_order):
            if self.layers[i] is None:
                ...
                if not original_bank:
                    projection_mat_aug = metas['projection_mat']
                    metas['projection_mat'] = metas['projection_mat_og']
                    instance_feature_original = self.layers[i](
                        instance_feature,
                        anchor,
                        anchor_embed,
                        metas['feature_map_og'],
                        metas,
                    )
                    metas['projection_mat'] = projection_mat_aug
                ...
                if not original_bank:
                    instance_feature_original = instance_feature_original.detach() 
                    
                    
                    # Cosine Similarity Loss
                    # cosine_sim = F.cosine_similarity(instance_feature, instance_feature_original, dim=2)  # [B, N]
                    # instance_loss = (1 - cosine_sim).mean()
                    instance_loss = F.mse_loss(instance_feature, instance_feature_original, reduction='none')
        ...
        if instance_loss is not None:
            score, _ = torch.sigmoid(classification[-1]).max(dim=-1)
            foreground = score > self.kd_threshold
            # import pdb; pdb.set_trace() 
            if not foreground.any():
                instance_loss = torch.tensor(0.0, device=score.device)    
            else:
                instance_loss = instance_loss[foreground].mean()

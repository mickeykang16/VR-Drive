...
# length = {'trainval': 26730, 'mini': 323}
length = {'trainval': 27430, 'mini': 323} # Only remove the first sample of each sequence
    ...
    head=dict(
        ...
        map_head=dict(
            ...
            operation_order=(
                ...
                * (num_decoder - num_single_frame_decoder_map-1)
            )[:],
            temp_graph_model=dict(
                type="MultiheadFlashAttention",
...
test_pipeline = [
    dict(type="LoadMultiViewImageFromFiles", to_float32=True),
    # TODO: Remove gt depth if unnecessary
    # dict(
    #     type="LoadPointsFromFile",
    #     coord_type="LIDAR",
    #     load_dim=5,
...
eval_config = dict(
    **data_basic_config,
    ann_file=anno_root + 'nuscenes_infos_val_truck_small_tilt.pkl', # For novel view
    # ann_file=anno_root + "nuscenes_infos_val.pkl", # original val full
...
data_aug_conf = {
    ...
    "bot_pct_lim": (0.0, 0.0),
    # "rot_lim": (-5.4, 5.4),
    "rot_lim": (0.0, 0.0), # keep extrinsic unchanged
    "H": 900,
    "W": 1600,
    "rand_flip": True,
    "rot3d_range": [0, 0], # keep extrinsic unchanged
}
...
data = dict(
    ...
    train=dict(
        **data_basic_config,
        # ann_file=anno_root + "nuscenes_infos_train_drivingforward.pkl",
        ...
        novel_view_config={
            ...
            'height': [-0.7, 1.0],
            'depth': [-0.2, 1.0]
            # cam pose change, [height, pitch, depth], refer to [Towards Viewpoint Robustness in Bird's Eye View Segmentation]    
        }
    ),
    val=dict(
        **data_basic_config,
        ann_file=anno_root + "nuscenes_infos_val_truck_small_tilt.pkl",
        # ann_file=anno_root + "nuscenes_infos_val.pkl",
        # nuscenes_infos_val_truck_small_tilt
    ...
    test=dict(
        **data_basic_config,
        ...
        novel_view=True,
        # novel_view_config={
        #     'img_dir':'images_truck_small_tilt',
        #     'cam_delta':[0.7, -7.0, 0.0], 
        #     # cam pose change, [height, pitch, depth], refer to [Towards Viewpoint Robustness in Bird's Eye View Segmentation]    
        # }
        # novel_view_config={
        #     'img_dir':'images_orig',
        #     'cam_delta':[0.0, 0.0, 0.0], 
        # }
        # novel_view_config={
        #     'img_dir':'images_height+1',
        #     'cam_delta':[1.0, 0.0, 0.0], 
        # }
        # novel_view_config={
        #     'img_dir':'images_height-0.7',
        #     'cam_delta':[-0.7, 0.0, 0.0], 
        # }
        novel_view_config={
            'img_dir':'images_pitch+5_low_light',
            'cam_delta':[0.0, 5.0, 0.0], 
        }
        # novel_view_config={
        #     'img_dir':'images_pitch-10',
        #     'cam_delta':[0.0, -10.0, 0.0], 
        # }
        # novel_view_config={
        #     'img_dir':'images_depth+1',
        #     'cam_delta':[0.0, 0.0, 1.0], 
        # }
    ),
)
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        ...
        custom_keys={
            ...
            # "losses": dict(lr_mult=0., decay_mult=0.),
            # "gaussian_branch": dict(lr_mult=0.1)
        }
    ),
)
...
#         custom_keys={
#             "img_backbone": dict(lr_mult=0.1),
#             "gaussian_branch": dict(lr_mult=1.0)
#         },
#         ),
...
# load_from = 'ckpt/diffusiondrive_stage2.pth'
# load_from = 'work_dirs/diffusion_drive_batch4_ours_depth_lr_5e5/latest.pth'
# load_from = 'work_dirs/ours_w_gaussian_v2/latest.pth'
load_from = '~/iter_66000.pth'
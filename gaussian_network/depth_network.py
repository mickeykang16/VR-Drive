...
class DepthNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.conv1x1 = conv2d(256*3, self.fusion_feat_in_dim, kernel_size=1, padding_mode = 'reflect') 

        # fusion net
        # fusion_feat_out_dim = self.encoder.num_ch_enc[self.fusion_level] 
        
        fusion_feat_out_dim = 128
        ...
        num_ch_enc = [64, 256, 128]
        num_ch_dec = [256, 256, 256]
    ...
    def forward(self, img_feats, inputs):
        '''
        ...
        img_feats = [inputs['feat_out']] + img_feats
        
        img_feats = [pack_cam_feat(feat) for feat in img_feats]
        ...
        _, _, up_h, up_w = img_feats[lev].size() 
 
        packed_feats_list_ = img_feats[lev:lev+1] \
                        + [F.interpolate(feat, [up_h, up_w], mode='bilinear', align_corners=True) for feat in img_feats[lev+1:]]        
        
        packed_feats_agg = self.conv1x1(torch.cat(packed_feats_list_, dim=1)) 
        ...
        feat_in = img_feats[:lev] + [fusion_dict['proj_feat']]   

...
class DepthNetwork(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.conv1x1_ = conv2d(512, self.fusion_feat_in_dim, kernel_size=1, padding_mode = 'reflect') 
    ...
    def forward(self, img_feats, inputs):
        ...
        packed_feats_agg_ = self.conv1x1_(torch.cat(packed_feats_list_, dim=1)) 

        feats_agg_ = unpack_cam_feat(packed_feats_agg_, self.batch_size, self.num_cams) 
        ...
        ####################
        fusion_dict = self.fusion_net_(inputs, feats_agg_)
        feat_in = packed_feats[:lev] + [fusion_dict['proj_feat']]   
        img_feat = []
        for i in range(len(feat_in)):
            img_feat.append(unpack_cam_feat(feat_in[i], self.batch_size, self.num_cams))

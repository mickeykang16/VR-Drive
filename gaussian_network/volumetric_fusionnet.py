...
class VFNet(nn.Module):
    ...
    def calculate_sample_pixel_coords(self, K, v_pts, w_dim, h_dim):
        ...
        """        
        cam_points = torch.matmul(K[:, :3, :3].float(), v_pts) # [batch_size, 3, n_voxels]
    ...
    def project_voxel_into_image(self, voxel_feat, inv_K, extrinsics):
        ...
        for cam in range(self.num_cams):
            ...
            cam_points = torch.matmul(inv_K[:, cam, :3, :3].float(), self.pixel_grid)
            ...
            points = torch.matmul(extrinsics[:, cam, :3, :].float(), cam_points)
